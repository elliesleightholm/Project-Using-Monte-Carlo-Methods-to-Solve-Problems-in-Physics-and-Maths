...
# Creating the parameters for the lattice, e.g size, temperature, etc
class Lattice:
...
# Neighbouring spins prefer to be aligned
# Low temperature
    def init_lowT(self):  # generates an array of ones
...
# High temperature - generates an array of 1's or -1's
    def init_highT(self):
        l = np.zeros((self.N, self.N), dtype=int)
        for y in range(self.N):
            for x in range(self.N):
    ...
    # calculates E (energy) for one element
    def E_elem(self, x, y):
...
class plots:
    def __init__(self, N=10, B=0, start='Low', x0=1, x1=5, inc=0.1, steps=50000, T=1):
        ...
        self.steps = steps
        self.T = T
        self.title = "Heat Capacity vs Temperature"
    ...
    def plot(self):
        ...
        plt.title(self.title)
        plt.plot(np.arange(self.x0, self.x1, self.inc), self.data, c='b', label="%s x %s Lattice"%(self.N, self.N))  # ,label='B field is {}'.format(self.B))
        plt.legend()
    ...
    def lattice(self):
        self.title = 'A 2D Lattice of Atomic Spins for a Fixed Temperature. \n {} Steps ' \
                     'Through the Metropolis Algorithm'.format(self.steps)
...
# we're just going to pretend i'm typing lol no one will ever know
...
P = I.plots(N=150, steps=100000000, start="High", T=0.4)
P.lattice()
P.show()

# for i in range(0, 1000000, 10000):
#     P = I.plots(N=150, steps=i, start="High")
#     P.lattice()
#     P.show()

# 2.269185 - critical temperature

# If we increase the 'steps', this will increase the number of times the metropolis
# algorithm is repeated and thus, we expect the lattice to tend towards order

# Changing the magnetic field to a negative will invert the colours
